...
Prevent sending HTTP requests with invalid request headers.
The default `raise` will result in an exception being raised
when invalid request headers are detected. When set to
`ignore` the check is not performed at all: use at your
own risk.
...
Parameters offered by the client when negotiating the
permessage-deflate Websocket extension (RFC 7692). All fields
are optional and only those set are included in the offer.

The `server_context_takeover`, `client_context_takeover`,
`server_max_window_bits` and `client_max_window_bits` fields
shape the `sec-websocket-extensions` request header offer.

For `client_max_window_bits`, the atom `bare` causes the
parameter to be offered without a value, advertising support
for the server to constrain the client's LZ77 sliding window
in its response (RFC 7692 Section 7.1.2.2). An integer in
`8..15` also self-imposes that limit on the client. Omitting
the field disables the parameter altogether.

The `level`, `mem_level` and `strategy` fields are local zlib
codec tuning options. They are forwarded to Cowlib when the
deflate context is initialized and have no wire representation.
Their value domains mirror Cowlib's `cow_ws:deflate_opts()`.
...
Parameters offered in the `sec-websocket-extensions` request header
when `compress` is enabled, plus local zlib codec tuning forwarded
to Cowlib. Has no effect when `compress` is `false`.
+
Setting this option overrides the default offer entirely.
In particular, `deflate_opts => #{}` advertises a bare
`permessage-deflate` with no parameters, not the historical
default. When the option is not set, Gun offers
`permessage-deflate; client_max_window_bits; server_max_window_bits=15`
to preserve backwards compatibility. See the `deflate_opts()` type
and RFC 7692 for details.
...
* *2.5*: The `deflate_opts` Websocket option and the
         `deflate_opts()` type were added.
