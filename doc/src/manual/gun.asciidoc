...
Maximum number of headers allowed per response. Note that
this is checked only after the full header block was
received. When the number of headers exceeds this limit
Gun closes the connection with a `limit_reached`
connection error.
...
+
The value `compat` will enable compression without sending
any parameter. It should help connecting to servers that
reject parameters that Gun sends by default.
...
including across fragments and after decompression.
Gun closes the connection when this limit is exceeded.
...
* *2.6*: The `max_headers` HTTP/1.1 option was added.
* *2.5*: The `max_headers` HTTP/2 option was added.
* *2.5*: Websocket `compress` option value `compat` was added.
