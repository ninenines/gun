...
Note that it is the responsibility of the custom backend
to provide cookies that conform to RFC6265 if they come
from external sources (e.g. files), especially for the
`name`, `value`, `domain` and `path` fields.
