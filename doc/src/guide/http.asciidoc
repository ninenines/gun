...
You can use an additional argument, that all request functions have,
to specify the pid of a process to redirect response handling to.

.GET "/organizations/ninenines" specifying a process to reply to

[source,erlang]
ReqOpts = #{reply_to => HandlerPid},
StreamRef = gun:get(ConnPid, "/organizations/ninenines", [], ReqOpts).
